...
import numpy as np
import weaver2.benchmarks
import weaver2.benchmarks.yolo2_tiny
import weaver2.compiler
import logging 
from weaver2.fpga.fpgamanager import FPGAManager
import weaver2.simulator.accelerator

def initialize_yolo_graph(weight_pickle, debug_mode=False):
    yolo_graph = weaver2.benchmarks.get_graph('yolo2_tiny', train=False)

    fpga_spec = weaver2.compiler.FPGASpec(num_ddr=1, size_ddr=1024, bandwidth_per_ddr=512)
    fpga_compiler = weaver2.compiler.GraphCompiler(fpga_spec)
    ...
    acc_obj = weaver2.simulator.accelerator.Accelerator(N=32,M=32,prec=16,mem_if_width=256,frequency=100e6,sram=sram)
